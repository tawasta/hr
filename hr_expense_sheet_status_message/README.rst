.. image:: https://img.shields.io/badge/licence-AGPL--3-blue.svg
   :target: http://www.gnu.org/licenses/agpl-3.0-standalone.html
   :alt: License: AGPL-3

================================================================
Send a message after Submitted and Approved states from Expenses
================================================================

"Submit to Manager" -button sends a customized message and "Approve" -button
also sends one.

Messages are modifiable from mail templates.

Configuration
=============
Modify mail templates as you see fit.

Usage
=====
Go to Expenses and select an expense report to submit and approve it.

Known issues / Roadmap
======================
This module does not disable existing messages which are sent from expense reports.
Consider disabling those messages if they cause confusion to users.

Credits
=======

Contributors
------------

* Timo Kekäläinen <timo.kekalainen@tawasta.fi>

Maintainer
----------

.. image:: http://tawasta.fi/templates/tawastrap/images/logo.png
   :alt: Oy Tawasta OS Technologies Ltd.
   :target: http://tawasta.fi/

This module is maintained by Oy Tawasta OS Technologies Ltd.
